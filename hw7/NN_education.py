...
for i in xrange(len(list1)):
    for j in xrange(num_attr+1):
        temp = float(list1[i][j])/100.0
        list1[i][j]= round(temp,2)
...
for i in xrange(len(list2)):
    for j in xrange(num_attr):
        temp=float(list2[i][j])/100.0
        list2[i][j]= round(temp,2)
    ...
    def __init__(self, num_input, num_hiden, num_output):
        ...
        for j in xrange(self.num_hiden):
            for i in xrange(self.num_input):
                self.w_ih[j][i] = random.uniform(-0.5,0.5)
    #print self.w_ih
        for k in xrange(self.num_output):
            for j in xrange(self.num_hiden):
                self.w_ho[k][j] = random.uniform(-0.5,0.5)
#print self.w_ho
    ...
    def train(self, inputs, truths):
        for i in xrange(len(inputs)):
            ...
            self.backpropagation([truths[i]], 0.025)
                #print self.w_ih
