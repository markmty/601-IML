...
all_content = lib_content+con_content
...
for i in xrange(len(all_content)):
    all_content[i]=all_content[i].lower()
...
def topwords(dict,n):
    templist=[]
    for key in dict:
        templist.append((dict[key],key))
    templist.sort(key=lambda tup:tup[0])
    templist.reverse()

    stopwords=[]
    for i in xrange(n):
        stopwords.append(templist[i][1])
    return stopwords

stopwords=topwords(all_dict,N)
#print stopwords

def rmv_stopwords(text,words):
    temp = []
    for word in text:
...
con_content=rmv_stopwords(con_content,stopwords)
lib_content=rmv_stopwords(lib_content,stopwords)
